...
pub fn login(
    db: &State<MongoRepo>,
    cookies: &CookieJar<'_>,
    login_info: Validated<Json<LoginInfo>>,
) -> Result<(), Status> {
    ...
    let maybe_user = db
        .users
        .find_one(doc! { "email": new_login.username }, None);
                ...
                Ok(Some(auth_info)) => {
                    ...
                    if hash == auth_info.password_hash {
                        ...
                        cookies.add_private(Cookie::new(
                            "user_id",
                            user.id.expect("No User ID available").to_hex(),
                        )); // Add the user's ID to private cookies
